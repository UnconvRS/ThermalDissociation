...
class image:
    ...
    def findOptimalScaleforTemplMatching(self,tmpl):
        ...
        #(_, maxLoc, r) = self.optimalScale_tmplMatch
        #(startX, startY) = (int(maxLoc[0] * r), int(maxLoc[1] * r))
        #(endX, endY) = (int((maxLoc[0] + tW) * r), int((maxLoc[1] + tH) * r))

        ## draw a bounding box around the detected result and display the image
        #cv2.rectangle(self.gray_img, (startX, startY), (endX, endY), (0, 0, 255), 2)
        #cv2.imshow("Image", self.gray_img)
        #cv2.waitKey(0)
    ...
    def TemplateMatching(self,tmpl):
        ...
        self.findOptimalScaleforTemplMatching(tmpl)
        ...
        #self.xml_annotator=annotator('si_crystals',self.img_filename,self.img_w,self.img_h,3)
        ...
        for pt in zip(*loc[::-1]):
            if mask[pt[1] + int(round(h / 2)), pt[0] + int(round(w / 2))] != 255:
                ...
                # add to the xml tree
                #si_xy=[pt[0],pt[1],pt[0]+w,pt[1]+h]
                #self.xml_annotator.add_object('si',si_xy)
             
        
        self.numSiCrystals=len(self.sICrystals)

        # remove duplicated patterns found
        ctrs_copy=copy.deepcopy(ctrs)
        rmIDs=[]
