...
class trajectory:
    ...
    def __DetermineConversionFactors(self):
        ...
        self.timeConvertor=1.0 if self.simSettings.SimUnits.time==self.simSettings.OutUnits.time else self.simSettings.SimUnits.time_conversions[timeConvString]  
        self.LengthConvertor=1.0 if self.simSettings.SimUnits.length==self.simSettings.OutUnits.length else self.simSettings.SimUnits.length_conversions[lengthConvString]
    ...
    def detect_crystalls(self,numImagesSkipped):
        ...
        for i in tqdm(range(0,num_images,numImagesSkipped)):
            for slice in self.slices:
                ...
                massSi_xyz=numTotalSi_xyz*(self.Si_unit.UnitVol*pow(self.simSettings.SimUnits.length_conversions["A2cm"],3))*self.Si_unit.density
...
class simulation():
    ...
    def collectWriteOutputData(self):
        ...
        for restart in self.restarts:
            dictn = {f'time {simSettings.get_instance().OutUnits.time}':restart.traj.simTime_OutUnits,'num_unit_cells':restart.traj.numSiCrystals,'mass_hydrate [gr]':restart.traj.massSi}
