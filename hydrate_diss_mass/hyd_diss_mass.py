...
from hydrate_diss_mass.modules.units import units
from hydrate_diss_mass.modules.settings import simSettings
from hydrate_diss_mass.modules.simulation import simulation

# Here the code begins
# set type of input
toi = 1  # 1:cli, 2: in source code
if toi == 1:
    # Get input form cli
    parser = argparse.ArgumentParser();
    # required arguments
    parser.add_argument("--SimDir", "-sd", type=str, required=True,
                        help='The path to the simulation directory where all restart folders are located (required)')
    ...
    # optional arguments
    parser.add_argument("--NumImagesSkipped", "-si", type=int, required=False, default=1,
                        help='The number of images skipped to speed up the calculations (optional), default=1')
